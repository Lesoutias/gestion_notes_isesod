...
import { formatNomComplet } from '../../services/etudiantService'
import { Button } from '../ui/Button'

export function EtudiantHeader({ title, subtitle }) {
  const dispatch = useDispatch()
  const user = useSelector((state) => state.auth.user)
  const etudiant = useSelector((state) => state.auth.etudiant)

  const displayName = etudiant ? formatNomComplet(etudiant) : user?.login
